...
function RoamingPage() {
  const { isRestricted, isDeactivated } = useLifecycleGuard();
    ...
    <div className="animate-fade-in pb-6">
      ...
      {isRestricted && (
        <section className="mt-5 px-5">
          <p className="text-xs text-muted-foreground">
            {isDeactivated
              ? "Reactivation required before using roaming services."
              : "Roaming purchases are paused while your line is suspended."}
          </p>
        </section>
      )}
      ...
      <div className="mt-5 space-y-2 px-5">
        {[
          ...
          <div key={b.n} className={`flex items-center justify-between rounded-2xl border border-border bg-card p-4 shadow-soft ${isRestricted ? "opacity-60" : ""}`}>
            <div>
